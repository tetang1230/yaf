...
$(function(){
	...
	$("#btn_submit").click(function(){
			...
			if(error >= 0){
				...
				if(add == 1){
					//添加
                    var url = '/back/ajaxaddact';
                    var postParams = formParams($(".maskContainer"));
                    var fileIds = ['actone', 'acttwo', 'actthree'];
					...
					//更新
						...
						var url = '/back/ajaxupdateact?id='+uid;
						...
						$.post('/back/ajaxupdateact?id='+uid, updateParams,function(data) {
		...
		$("#add , #edit").click(function(){
			...
			if($(this).attr('id') == 'edit'){
				...
				$.getJSON("/back/ajaxact?id="+uid,function(jsonData){
		...
		$(".closeMask").click(function(){
			...
			if($(this).attr('id') == 'btn_deleteConfirm'){
				$.getJSON("/back/ajaxdeleteact?id="+uid,function(jsonData){
					if(jsonData == false){
						alert('删除失败');
					}
				});
		...
		new uploadPreview({ UpBtn: "actone",  ImgShow: "imgone"});
		new uploadPreview({ UpBtn: "acttwo",  ImgShow: "imgtwo"});
		new uploadPreview({ UpBtn: "actthree",  ImgShow: "imgthree"});
